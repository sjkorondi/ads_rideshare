...
class GUI():
    def __init__(self, root, cars, total, indiv):
        self.root = root
        self.distance = 0
        self.petrol = 0
        self.cars = cars
    ...
    def totalMilesDriven(self):
        ...
        tk.Label(self.root, 
                bg="white", 
                font=("Arial", 16, "bold"), 
                text="To cover the cost of petrol, we would need to charge €{} per kilometer driven.".format(round(self.petrol/self.distance, 2))
                ).pack(fill=tk.BOTH,expand=True)
        
        tk.Label(self.root, 
                bg="white", 
                font=("Arial", 16, "bold"), 
                text="If our forecasted (mean) total distance of ",
                ).pack(fill=tk.BOTH,expand=True)
    ...
    def indivCars(self):
        ...
        tk.Label(self.root, 
                ...
                text="These are the distances driven by each individual vehicle over the past three years." \
                    "\n Each car drove an average of {} kilometers per year.".format(avg_distance)
                ).pack(fill=tk.BOTH,expand=True)
